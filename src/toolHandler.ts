...
import { PressKeyTool } from './tools/browser/interaction.js';
...
function initializeTools(server: any) {
  ...
  // API tools
  if (!httpRequestTool) httpRequestTool = new HttpRequestTool(server);
...
export async function handleToolCall(
  ...
  if (BROWSER_TOOLS.includes(name)) {
    ...
    switch (name) {
      ...
      // API tools
      case "playwright_http_request":
        return await httpRequestTool.execute(args, context);
